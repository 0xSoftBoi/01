...
SANDBOX_PID=$!
trap 'kill $SANDBOX_PID ${JSONAPI_PID:-} ${FILL_PID:-} ${ACCRUE_PID:-} 2>/dev/null || true' EXIT
sleep 10
...
# 3. Operator automation: auto-fill purchase orders + nightly late-interest
#    accrual. Both run under the KYD-Operator party.
